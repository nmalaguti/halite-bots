...
public class GameMap implements Iterable<Location> {
    ...
    public GameMap(int width_, int height_) {
        ...
        for(int y = 0; y < height; y++) {
            ...
            for(int x = 0; x < width; x++) {
                row.add(new Site(0, 0, 0));
            }
            contents.add(row);
        }
    }

    public GameMap(int width_, int height_, ArrayList<ArrayList<Site>> contents_) {
        width = width_;
        height = height_;
        contents = contents_;
    }
    ...
    public GameMap(GameMap gameMap) {
        ...
        for (ArrayList<Site> row : gameMap.contents) {
            ...
            for (Site site : row) {
                newRow.add(new Site(site.getStrength(), site.getProduction(), site.getOwner()));
            }
            contents.add(newRow);
        }
    }
