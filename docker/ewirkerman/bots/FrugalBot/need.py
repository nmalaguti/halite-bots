		...
		if self.strength + gen_strength > self.effective_str:
			...
			if self.site.strength == 0:
				...
				for i in range(len(generation)):
					combo = [move for move in next(itertools.combinations(generation, i+1))]
...
#					need_logger.debug("Pledged %s from %s" % (site.strength,move.loc))
				...
				met = True
		elif self.strength + gen_strength + gen_production > self.effective_str:
			for move in generation:
				this_gen.append(Move(move.loc,0))
			met = True
		else:
			this_gen.extend(generation)
			self.production += gen_production
