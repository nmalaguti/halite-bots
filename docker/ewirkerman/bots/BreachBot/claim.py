...
#			logger.debug("%s expanding off checker because it's safe to do so" % claim)
		...
		elif will_move and claim.gen == 3 and parent.root.is_uncapped() and parent.root.cap and not claim.root.breakthrough:
#			logger.debug("staying off the first gen of a breakthrough at %s" % parent.root.loc)
			# loc.site.heap.filter_to_loc(parent.loc)
			claim.site.heap.dir = STILL
			return True
...
#		logger.debug( "Found breach_str of %s" % (self.breach_str))
	...
	breach = parent.is_root() and parent.site.strength and parent.site.strength < self.breach_str and self.site.strength and self.site.strength <= parent.site.strength + self.breach_str
...
#						# # logger.debug("Waste prevention child: %s" % child)
						pass
					elif self.is_uncapped() and self.cap and any([not move.loc.site.strength for move in child_site.neutrals]):
#						logger.debug("Not really a breach claim")
						pass
					else:
						child = parent.create_child(dir)
						current_children.add(child)
	...
	def __init__(self, map, location = None, parent = None, dir=0, root = None, gen = None):
		...
		if self.root is self:
			...
			if self.site.strength:
				self.cap = self.gameMap.get_friendly_strength(loc=self.loc, dist=6, type="enemies")
				self.value = 1000
			else:
